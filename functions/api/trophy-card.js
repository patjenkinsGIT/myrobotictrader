...
export async function onRequest(context) {
  ...
  try {
    ...
    const url = new URL(context.request.url);
    const debug = url.searchParams.get("debug") === "true";
    const fallbackData = getFallbackData(url.searchParams.get("type") || "month");
    const format = url.searchParams.get("format") || "html";
    
    // If debug mode and error, show error details
    if (debug) {
      return new Response(JSON.stringify({ error: error.message, fallback: fallbackData }, null, 2), {
        headers: {
          ...corsHeaders,
          "Content-Type": "application/json",
          "Cache-Control": "no-cache",
    ...
    if (format === "svg") {
      const svg = generateTrophyCardSVG(fallbackData, url.searchParams.get("type") || "month");
      return new Response(svg, {
        headers: {
          ...corsHeaders,
          "Content-Type": "image/svg+xml",
      ...
      const html = generateTrophyCardHTML(fallbackData, url.searchParams.get("type") || "month");
      return new Response(html, {
        headers: {
          ...corsHeaders,
          "Content-Type": "text/html",
...
async function fetchTrophyData(context, type) {
  ...
  if (!SHEETS_API_KEY || !SPREADSHEET_ID) {
    throw new Error(`Missing API credentials - Debug: ${JSON.stringify(debugInfo)}`);
  }
  ...
  if (rows.length < 5) {
    throw new Error(`Insufficient data in Records tab - found ${rows.length} rows, need 5`);
  }
...
function generateTrophyCardSVG(data, type) {
  ...
  <!-- Title -->
  <text x="300" y="125" text-anchor="middle" font-size="28" font-weight="bold" fill="url(#goldGradient)" font-family="Arial, sans-serif" letter-spacing="1.5">${titles[type]}</text>
  
  <!-- Subtitle -->
  <text x="300" y="148" text-anchor="middle" fill="rgba(255,255,255,0.7)" font-size="13" font-weight="600" font-family="Arial, sans-serif" letter-spacing="2">${subtitles[type].toUpperCase()}</text>
  
  <!-- Date/Period -->
  <text x="300" y="178" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="17" font-weight="600" font-family="Arial, sans-serif">${data.date}</text>
  
  <!-- Profit amount (adjusted spacing) -->
  <text x="300" y="220" text-anchor="middle" fill="#10b981" font-size="44" font-weight="bold" font-family="Arial, monospace">${data.profit.toFixed(2)}</text>
  ...
  <g transform="translate(${isNewRecord ? '115' : '155'}, 260)">
    ...
    <text x="70" y="28" text-anchor="middle" fill="white" font-size="22" font-weight="bold" font-family="Arial, monospace">${data.trades}</text>
    ...
    <text x="240" y="28" text-anchor="middle" fill="#10b981" font-size="22" font-weight="bold" font-family="Arial, monospace">+${data.beatBy.toFixed(2)}</text>
    ...
    <text x="225" y="28" text-anchor="middle" fill="white" font-size="18" font-weight="bold" font-family="Arial, monospace">${data.previous.toFixed(2)}</text>
