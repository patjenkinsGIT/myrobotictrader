...
async function fetchTrophyData(context, type) {
  ...
  if (rows.length < 5) {
    throw new Error(
      "Insufficient data in Records tab - found " +
        rows.length +
        " rows, need 5"
    );
  }
...
function getFallbackData(type) {
  ...
  const fallback = {
    ...
    month: {
      ...
      beatBy: 340.14,
      previous: 477.17,
      lastUpdated: "9/30/2025, 8:53:29 AM",
    },
  };
...
function generateTrophyCardSVG(data, type) {
  ...
  // Return pure SVG for embedding
  return `<svg width="600" height="350" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 350">
  <defs>
    <!-- Background gradient -->
